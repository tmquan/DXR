...
class DXRLightningModule(LightningModule):
    ...
    def _common_step(self, batch, batch_idx, optimizer_idx, stage: Optional[str] = "evaluation"):
        ...
        # Run the forward
        figure_dx_output = self.unet2d_model(x=torch.cat([figure_ct_interp, figure_xr_interp]), timesteps=timesteps, context=torch.cat([pose_random, pose_hidden]),)
        ...
        im2d_loss = self.l1loss(figure_ct_target, figure_ct_output) + self.l1loss(figure_xr_target, figure_xr_output) + self.l1loss(figure_ct_approx, figure_ct_random) + self.l1loss(figure_xr_approx, figure_xr_hidden)
        ...
        if batch_idx == 0:
            with torch.no_grad():
                ...
                figure_ct_sample = self.inferer.sample(input_noise=figure_ct_latent, diffusion_model=self.unet2d_model, scheduler=self.ddimsch, conditioning=pose_random,)

                figure_xr_latent = torch.randn_like(figure_xr_hidden)
                figure_xr_sample = self.inferer.sample(input_noise=figure_xr_latent, diffusion_model=self.unet2d_model, scheduler=self.ddimsch, conditioning=pose_hidden,)

                volume_dx_sample = self.forward_volume(image2d=torch.cat([figure_ct_sample, figure_xr_sample]), cameras=join_cameras_as_batch([view_random, view_hidden]), n_views=[1, 1],)
                ...
                viz2d = torch.cat(
                    [
                        torch.cat([image3d[..., self.vol_shape // 2, :], figure_ct_random, figure_ct_output, volume_ct_approx[..., self.vol_shape // 2, :], figure_ct_approx, volume_ct_sample[..., self.vol_shape // 2, :], figure_ct_sample,], dim=-2,).transpose(2, 3),
                        torch.cat([volume_xr_approx[..., self.vol_shape // 2, :], figure_xr_hidden, figure_xr_output, volume_xr_approx[..., self.vol_shape // 2, :], figure_xr_approx, volume_xr_sample[..., self.vol_shape // 2, :], figure_xr_sample,], dim=-2,).transpose(2, 3),
                    ],
                    dim=-2,
                )
