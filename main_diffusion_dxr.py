...
class DXRLightningModule(LightningModule):
    ...
    def _common_step(self, batch, batch_idx, optimizer_idx, stage: Optional[str] = "evaluation"):
        ...
        volume_dx_approx = self.forward_volume(
            image2d=torch.cat([figure_ct_random, image2d]),
        ...
        figure_ct_interp = self.ddpmsch.add_noise(
        ...
        figure_xr_interp = self.ddpmsch.add_noise(
        ...
        figure_dx_output = self.unet2d_model(
            x=torch.cat([figure_ct_interp, figure_xr_interp]),
            timesteps=timesteps,
        ...
        if batch_idx == 0:
            with torch.no_grad():
                ...
                figure_ct_sample = self.inferer.sample(
                    input_noise=figure_ct_latent,
                ...
                figure_xr_sample = self.inferer.sample(
                    input_noise=figure_xr_latent,
                    diffusion_model=self.unet2d_model,
                    scheduler=self.ddimsch,
