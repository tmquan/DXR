...
class DXRLightningModule(LightningModule):
    def __init__(self, hparams, **kwargs):
        ...
        self.inv_renderer = nn.Sequential(
            Unet(spatial_dims=2, in_channels=1, out_channels=self.n_pts_per_ray, channels=[64, 128, 256, 512, 1024], strides=(2, 2, 2, 2, 2), num_res_units=2, kernel_size=3, up_kernel_size=3, act=("LeakyReLU", {"inplace": True}), norm=Norm.BATCH, dropout=0.5,),
            Reshape(1, self.n_pts_per_ray, self.img_shape, self.img_shape),
            Unet(spatial_dims=3, in_channels=1, out_channels=1, channels=[16, 32, 64, 128, 256], strides=(2, 2, 2, 2, 2), num_res_units=2, kernel_size=3, up_kernel_size=3, act=("LeakyReLU", {"inplace": True}), norm=Norm.BATCH, dropout=0.5,),
        )

        self.unet3d_model = DiffusionModelUNet(spatial_dims=3, in_channels=1, out_channels=1, num_channels=[16, 32, 64, 128, 256], attention_levels=[False, False, False, True, True], norm_num_groups=16, num_res_blocks=2,)
    ...
    def forward_volume(self, image2d, cameras, n_views=[2, 1]):
        ...
        values = F.grid_sample(frustum, gpoints.view(-1, self.vol_shape, self.vol_shape, self.vol_shape, 3), mode="bilinear", padding_mode="zeros", align_corners=False,)
    ...
    def _common_step(self, batch, batch_idx, optimizer_idx, stage: Optional[str] = "evaluation"):
        ...
        volume_dx_approx = self.forward_volume(image2d=torch.cat([figure_ct_random, image2d]), cameras=join_cameras_as_batch([view_random, view_hidden]), n_views=[1, 1],)
        ...
        volume_ct_interp = self.ddpmsch.add_noise(original_samples=volume_ct_approx, noise=volume_ct_latent, timesteps=timesteps,)

        volume_xr_latent = torch.randn_like(image3d)
        volume_xr_interp = self.ddpmsch.add_noise(original_samples=volume_xr_approx, noise=volume_xr_latent, timesteps=timesteps,)
        ...
        im3d_loss = self.l1loss(volume_ct_target, volume_ct_output) + self.l1loss(volume_ct_approx, image3d)

        self.log(
            f"{stage}_im3d_loss", im3d_loss, on_step=(stage == "train"), prog_bar=True, logger=True, sync_dist=True, batch_size=self.batch_size,
        )

        im2d_loss = self.l1loss(figure_ct_target, figure_ct_output) + self.l1loss(figure_xr_target, figure_xr_output) + self.l1loss(figure_ct_approx, figure_ct_random) + self.l1loss(figure_xr_approx, image2d)
        ...
        if batch_idx == 0:
            with torch.no_grad():
                volume_ct_latent = torch.randn_like(image3d)
                volume_ct_sample = self.inferer.sample(input_noise=volume_ct_latent, diffusion_model=self.unet3d_model, scheduler=self.ddimsch,)
                figure_ct_sample = self.forward_screen(image3d=volume_ct_sample, cameras=view_random)

                volume_xr_latent = torch.randn_like(image3d)
                volume_xr_sample = self.inferer.sample(input_noise=volume_xr_latent, diffusion_model=self.unet3d_model, scheduler=self.ddimsch,)
                figure_xr_sample = self.forward_screen(image3d=volume_xr_sample, cameras=view_hidden)
                viz2d = torch.cat(
                    [
                        torch.cat(
                            [image3d[..., self.vol_shape // 2, :], figure_ct_random, volume_ct_output[..., self.vol_shape // 2, :], figure_ct_output, volume_ct_approx[..., self.vol_shape // 2, :], figure_ct_approx, volume_ct_sample[..., self.vol_shape // 2, :], figure_ct_sample,], dim=-2,
                        ).transpose(2, 3),
                        torch.cat(
                            [volume_xr_approx[..., self.vol_shape // 2, :], figure_xr_hidden, volume_xr_output[..., self.vol_shape // 2, :], figure_xr_output, volume_xr_approx[..., self.vol_shape // 2, :], figure_xr_approx, volume_xr_sample[..., self.vol_shape // 2, :], figure_xr_sample,], dim=-2,
                        ).transpose(2, 3),
                    ],
                    dim=-2,
                )
