...
class DXRLightningModule(LightningModule):
    ...
    def _common_step(self, batch, batch_idx, optimizer_idx, stage: Optional[str] = 'evaluation'):
        ...
        figure_dx_concat_output = self.unet2d_model(
            x=torch.cat([figure_ct_random_interp, figure_ct_hidden_interp, figure_xr_hidden_interp]),
            timesteps=timesteps, 
            context=torch.cat([pose_random, pose_hidden, pose_hidden]))
        figure_ct_random_output,\
        figure_ct_hidden_output,\
        figure_xr_hidden_output = torch.split(figure_dx_concat_output, batchsz)
        ...
        im2d_loss_dir = self.l1loss(figure_xr_hidden_hidden, figure_xr_hidden) \
