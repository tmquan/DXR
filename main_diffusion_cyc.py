...
class DXRLightningModule(LightningModule):
    ...
    def _common_step(self, batch, batch_idx, optimizer_idx, stage: Optional[str] = "evaluation"):
        ...
        volume_dx_concat = self.forward_volume(
            image2d=torch.cat([figure_ct_random, figure_ct_hidden, figure_xr_hidden]), cameras=join_cameras_as_batch([view_random, view_hidden, view_hidden]), n_views=[1, 1, 1],
        )
        ...
        figure_dx_concat_output = self.unet2d_model(
            x=torch.cat([figure_ct_random_interp, figure_ct_hidden_interp, figure_xr_hidden_interp,]), timesteps=timesteps, context=torch.cat([pose_random, pose_hidden, pose_hidden]),
        )
        ...
        if batch_idx == 0:
            ...
            viz2d = torch.cat(
                [
                    torch.cat(
                        [image3d[..., self.vol_shape // 2, :], figure_ct_random, figure_ct_hidden, figure_ct_random_random, figure_ct_random_hidden, figure_ct_hidden_random, figure_ct_hidden_hidden,],
                        dim=-2,
                    ).transpose(2, 3),
                    torch.cat(
                        [
