...
class DXRLightningModule(LightningModule):
    ...
    def _common_step(self, batch, batch_idx, optimizer_idx, stage: Optional[str] = "evaluation"):
        ...
        volume_dx_concat = self.forward_volume(image2d=torch.cat([figure_ct_random, figure_ct_hidden, figure_xr_hidden]), cameras=join_cameras_as_batch([view_random, view_hidden, view_hidden]), n_views=[1, 1, 1],)
        ...
        # Run the forward
        figure_dx_concat_output = self.unet2d_model(x=torch.cat([figure_ct_random_interp, figure_ct_hidden_interp, figure_xr_hidden_interp,]), timesteps=timesteps, context=torch.cat([pose_random, pose_hidden, pose_hidden]),)
        (figure_ct_random_output, figure_ct_hidden_output, figure_xr_hidden_output,) = torch.split(figure_dx_concat_output, batchsz)
        ...
        if batch_idx == 0:
            ...
            with torch.no_grad():
                ...
                # volume_dx_concat_sample = self.forward_volume(
                #     image2d=torch.cat([figure_dx_random_sample, figure_dx_hidden_sample]),
        ...
        # im2d_loss_cyc = self.l1loss(figure_dx_random_sample_random, figure_ct_random) \
        #               + self.l1loss(figure_dx_random_sample_hidden, figure_ct_hidden) \
        #               + self.l1loss(figure_dx_hidden_sample_random, figure_ct_random) \
        #               + self.l1loss(figure_dx_hidden_sample_hidden, figure_ct_hidden)
        ...
        im2d_loss = im2d_loss_dif + im2d_loss_dir
        self.log(
            f"{stage}_im2d_loss", im2d_loss, on_step=(stage == "train"), prog_bar=True, logger=True, sync_dist=True, batch_size=self.batch_size,
        )

        im3d_loss = self.l1loss(volume_ct_random, image3d) + self.l1loss(volume_ct_hidden, image3d)
        self.log(
        ...
        if batch_idx == 0:
            ...
            viz2d = torch.cat(
                [
                    torch.cat([image3d[..., self.vol_shape // 2, :], figure_ct_random, figure_ct_hidden, figure_ct_random_random, figure_ct_random_hidden, figure_ct_hidden_random, figure_ct_hidden_hidden,], dim=-2,).transpose(2, 3),
                    torch.cat(
                        [
                            image2d,
                            ...
                            figure_dx_random_sample,
                            figure_dx_hidden_sample,
                            # volume_dx_random_sample[..., self.vol_shape//2, :],
                            # volume_dx_hidden_sample[..., self.vol_shape//2, :],
                            # figure_dx_random_sample_random,
                            # figure_dx_random_sample_hidden,
                            # figure_dx_hidden_sample_random,
                            # figure_dx_hidden_sample_hidden,
                        ],
                        dim=-2,
                    ).transpose(2, 3),
                ],
