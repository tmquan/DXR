...
class DXRLightningModule(LightningModule):
    def __init__(self, hparams, **kwargs):
        ...
        self.fwd_renderer = DirectVolumeFrontToBackRenderer(image_width=self.img_shape, image_height=self.img_shape, n_pts_per_ray=self.n_pts_per_ray, min_depth=4.0, max_depth=8.0, ndc_extent=4.0,)
        ...
        self.unet2d_model = DiffusionModelUNet(spatial_dims=2, in_channels=1, out_channels=1, num_channels=backbones[self.backbone], attention_levels=[False, False, False, True, True], norm_num_groups=16, num_res_blocks=2, with_conditioning=True, cross_attention_dim=12,)  # flatR | flatT
        ...
        self.ddimsch = DDIMScheduler(num_train_timesteps=self.timesteps, schedule="scaled_linear_beta", prediction_type=hparams.prediction_type, beta_start=0.0005, beta_end=0.0195, clip_sample=False,)
    ...
    def _common_step(self, batch, batch_idx, optimizer_idx, stage: Optional[str] = "evaluation"):
        ...
        volume_dx_concat = self.forward_volume(image2d=torch.cat([figure_xr_hidden, figure_ct_random, figure_ct_hidden]), cameras=join_cameras_as_batch([view_hidden, view_random, view_hidden]), n_views=[1, 1, 1],)
        ...
        im2d_loss_inv = (
            self.l1loss(figure_xr_hidden_hidden, figure_xr_hidden) + self.l1loss(figure_ct_random_random, figure_ct_random) + self.l1loss(figure_ct_random_hidden, figure_ct_hidden) + self.l1loss(figure_ct_hidden_random, figure_ct_random) + self.l1loss(figure_ct_hidden_hidden, figure_ct_hidden)
        )

        im3d_loss_inv = self.l1loss(volume_ct_random, image3d) + self.l1loss(volume_ct_hidden, image3d)
        ...
        figure_xr_hidden_interp = self.ddpmsch.add_noise(original_samples=figure_xr_hidden, noise=figure_xr_latent, timesteps=timesteps,)

        figure_ct_latent = torch.randn_like(image2d)
        figure_ct_random_interp = self.ddpmsch.add_noise(original_samples=figure_ct_random, noise=figure_ct_latent, timesteps=timesteps,)
        figure_ct_hidden_interp = self.ddpmsch.add_noise(original_samples=figure_ct_hidden, noise=figure_ct_latent, timesteps=timesteps,)

        # Run the forward
        figure_dx_concat_output = self.unet2d_model(x=torch.cat([figure_xr_hidden_interp, figure_ct_random_interp, figure_ct_hidden_interp,]), timesteps=timesteps, context=torch.cat([pose_hidden, pose_random, pose_hidden]),)

        # Add forward backward reconstruction module
        volume_dx_concat_output = self.forward_volume(image2d=figure_dx_concat_output, cameras=join_cameras_as_batch([view_hidden, view_random, view_hidden]), n_views=[1, 1, 1],)
        figure_dx_concat_output = self.forward_screen(image3d=volume_dx_concat_output, cameras=join_cameras_as_batch([view_hidden, view_random, view_hidden]),)
        (volume_xr_hidden_output, volume_ct_random_output, volume_ct_hidden_output,) = torch.split(volume_dx_concat_output, batchsz)
        (figure_xr_hidden_output, figure_ct_random_output, figure_ct_hidden_output,) = torch.split(figure_dx_concat_output, batchsz)

        if self.ddpmsch.prediction_type == "v_prediction":
            figure_xr_hidden_target = self.ddpmsch.get_velocity(figure_xr_hidden, figure_xr_latent, timesteps)
            figure_ct_random_target = self.ddpmsch.get_velocity(figure_ct_random, figure_ct_latent, timesteps)
            figure_ct_hidden_target = self.ddpmsch.get_velocity(figure_ct_hidden, figure_ct_latent, timesteps)
        elif self.ddpmsch.prediction_type == "epsilon":
            figure_xr_hidden_target = figure_xr_latent
            figure_ct_random_target = figure_ct_latent
            figure_ct_hidden_target = figure_ct_latent
        elif self.ddpmsch.prediction_type == "sample":
            volume_ct_random_target = image3d
            volume_ct_hidden_target = image3d
            figure_xr_hidden_target = figure_xr_hidden
            figure_ct_random_target = figure_ct_random
            figure_ct_hidden_target = figure_ct_hidden

        im2d_loss_dif = self.l1loss(figure_xr_hidden_target, figure_xr_hidden_output) + self.l1loss(figure_ct_random_target, figure_ct_random_output) + self.l1loss(figure_ct_hidden_target, figure_ct_hidden_output)

        im3d_loss_dif = self.l1loss(volume_ct_random_target, volume_ct_random_output) + self.l1loss(volume_ct_hidden_target, volume_ct_hidden_output)
        ...
        if batch_idx == 0:
            # Sampling step
            with torch.no_grad():
                figure_dx_latent = torch.randn_like(image2d)
                figure_dx_sample = self.inferer.sample(input_noise=torch.cat([figure_dx_latent, figure_dx_latent]), diffusion_model=self.unet2d_model, scheduler=self.ddimsch, conditioning=torch.cat([pose_random, pose_hidden]), verbose=False,)
                figure_dx_random_sample, figure_dx_hidden_sample = torch.split(figure_dx_sample, batchsz)

                # Cycle consistent reconstruction
                volume_dx_concat_sample = self.forward_volume(image2d=torch.cat([figure_dx_random_sample, figure_dx_hidden_sample]), cameras=join_cameras_as_batch([view_random, view_hidden]), n_views=[1, 1],)
                volume_dx_random_sample, volume_dx_hidden_sample = torch.split(volume_dx_concat_sample, batchsz)
                figure_dx_random_sample_random = self.forward_screen(image3d=volume_dx_random_sample, cameras=view_random)
                figure_dx_random_sample_hidden = self.forward_screen(image3d=volume_dx_random_sample, cameras=view_hidden)
                figure_dx_hidden_sample_random = self.forward_screen(image3d=volume_dx_hidden_sample, cameras=view_random)
                figure_dx_hidden_sample_hidden = self.forward_screen(image3d=volume_dx_hidden_sample, cameras=view_hidden)
            ...
            viz2d = torch.cat(
                [
                    torch.cat([image2d, volume_xr_hidden[..., self.vol_shape // 2, :], figure_xr_hidden_hidden, image3d[..., self.vol_shape // 2, :], figure_ct_random, figure_ct_hidden,], dim=-2,).transpose(2, 3),
                    torch.cat([volume_ct_random[..., self.vol_shape // 2, :], figure_ct_random_random, figure_ct_random_hidden, volume_ct_hidden[..., self.vol_shape // 2, :], figure_ct_hidden_random, figure_ct_hidden_hidden,], dim=-2,).transpose(2, 3),
                    torch.cat([volume_dx_random_sample[..., self.vol_shape // 2, :], figure_dx_random_sample_random, figure_dx_random_sample_hidden, volume_dx_hidden_sample[..., self.vol_shape // 2, :], figure_dx_hidden_sample_random, figure_dx_hidden_sample_hidden,], dim=-2,).transpose(2, 3),
                ],
                dim=-2,
            )
