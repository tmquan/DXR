...
class DXRLightningModule(LightningModule):
    ...
    def _common_step(self, batch, batch_idx, optimizer_idx, stage: Optional[str] = "evaluation"):
        ...
        figure_xr_hidden_interp = self.ddpmsch.add_noise(
            original_samples=figure_xr_hidden,
            noise=figure_xr_latent,
            timesteps=timesteps,
        )

        figure_ct_latent = torch.randn_like(image2d)
        figure_ct_random_interp = self.ddpmsch.add_noise(
            original_samples=figure_ct_random,
            noise=figure_ct_latent,
            timesteps=timesteps,
        )
        figure_ct_hidden_interp = self.ddpmsch.add_noise(
            original_samples=figure_ct_hidden,
            noise=figure_ct_latent,
            timesteps=timesteps,
        )
        ...
        figure_dx_concat_output = self.unet2d_model(
            x=torch.cat(
                [
                    figure_xr_hidden_interp,
                    figure_ct_random_interp,
                    figure_ct_hidden_interp,
                ]
            ),
            timesteps=timesteps,
        ...
        (
            ...
            volume_ct_hidden_output,
        ) = torch.split(volume_dx_concat_output, batchsz)
        (
            figure_xr_hidden_output,
            figure_ct_random_output,
            figure_ct_hidden_output,
        ) = torch.split(figure_dx_concat_output, batchsz)
