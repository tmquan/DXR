...
class DXRLightningModule(LightningModule):
    ...
    def _common_step(self, batch, batch_idx, optimizer_idx, stage: Optional[str] = 'evaluation'):
        ...
        figure_dx_concat_output = self.unet2d_model(
            x=torch.cat([figure_xr_hidden_interp, figure_ct_random_interp, figure_ct_hidden_interp]),
            timesteps=timesteps, 
            context=torch.cat([pose_hidden, pose_random, pose_hidden]))
        
        # Add forward backward reconstruction module
        volume_dx_concat_output = self.forward_volume(
            image2d=figure_dx_concat_output,
            cameras=join_cameras_as_batch([view_hidden, view_random, view_hidden]),
            n_views=[1, 1, 1])
        figure_dx_concat_output = self.forward_screen(
            image3d=volume_dx_concat_output, 
            cameras=join_cameras_as_batch([view_hidden, view_random, view_hidden])
        )
        volume_xr_hidden_output,\
        volume_ct_random_output,\
        volume_ct_hidden_output = torch.split(volume_dx_concat_output, batchsz)
        figure_xr_hidden_output,\
        figure_ct_random_output,\
        figure_ct_hidden_output = torch.split(figure_dx_concat_output, batchsz)
        ...
        im2d_loss_dif = self.l1loss(figure_xr_hidden_target, figure_xr_hidden_output) \
                      + self.l1loss(figure_ct_random_target, figure_ct_random_output) \
                      + self.l1loss(figure_ct_hidden_target, figure_ct_hidden_output) 
        
        im3d_loss_dif = self.l1loss(volume_ct_random_target, volume_ct_random_output) \
                      + self.l1loss(volume_ct_hidden_target, volume_ct_hidden_output) 
