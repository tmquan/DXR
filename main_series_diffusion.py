...
class DXRLightningModule(LightningModule):
    ...
    def _common_step(self, batch, batch_idx, optimizer_idx, stage: Optional[str] = "evaluation"):
        ...
        volume_dx_concat = self.forward_volume(
            image2d=torch.cat([figure_xr_hidden, figure_ct_random, figure_ct_hidden]), cameras=join_cameras_as_batch([view_hidden, view_random, view_hidden]), n_views=[1, 1, 1],
        )
        ...
        # Run the forward
        figure_dx_concat_output = self.unet2d_model(
            x=torch.cat([figure_xr_hidden_interp, figure_ct_random_interp, figure_ct_hidden_interp,]), timesteps=timesteps, context=torch.cat([pose_hidden, pose_random, pose_hidden]),
        )
        ...
        im2d_loss_dif = (
            self.l1loss(figure_xr_hidden_target, figure_xr_hidden_output)
            + self.l1loss(figure_ct_random_target, figure_ct_random_output)
            + self.l1loss(figure_ct_hidden_target, figure_ct_hidden_output)
        )
        ...
        if batch_idx == 0:
            ...
            with torch.no_grad():
                ...
                figure_dx_sample = self.inferer.sample(
                    input_noise=torch.cat([figure_dx_latent, figure_dx_latent]),
                    diffusion_model=self.unet2d_model,
                    scheduler=self.ddimsch,
                    conditioning=torch.cat([pose_random, pose_hidden]),
                    verbose=False,
                )
                ...
                volume_dx_concat_sample = self.forward_volume(
                    image2d=torch.cat([figure_dx_random_sample, figure_dx_hidden_sample]), cameras=join_cameras_as_batch([view_random, view_hidden]), n_views=[1, 1],
                )
            ...
            viz2d = torch.cat(
                [
                    torch.cat(
                        [image2d, volume_xr_hidden[..., self.vol_shape // 2, :], figure_xr_hidden_hidden, image3d[..., self.vol_shape // 2, :], figure_ct_random, figure_ct_hidden,], dim=-2,
                    ).transpose(2, 3),
                    torch.cat(
                        [
                            ...
                            figure_ct_hidden_random,
                            figure_ct_hidden_hidden,
                        ],
                        dim=-2,
                    ).transpose(2, 3),
                    torch.cat(
