...
class DXRLightningModule(LightningModule):
    ...
    def training_step(self, batch, batch_idx, optimizer_idx=None):
        ...
        if batch_idx%2==0:
            ...
            im2d_loss_inv = self.l1loss(figure_ct_random_inverse_random, figure_ct_random) \
            ...
            if self.lpips:
                ...
                self.log(f'train_lpip_loss', lpips_loss, on_step=True, prog_bar=True, logger=True, sync_dist=True, batch_size=self.batch_size)
            ...
            im2d_loss_dif = self.l1loss(figure_xr_output_hidden, figure_xr_target_hidden) \
                          ...
                          + self.l1loss(figure_ct_random_output_random, figure_ct_target_random) \
                          + self.l1loss(figure_ct_random_output_hidden, figure_ct_target_hidden) \
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if (batch_idx%2)==0:
            ...
            if self.lpips:
                ...
                lpips_loss = self.lpips_(figure_xr_hidden_inverse_random.repeat(1, 3, 1, 1).clamp(-1, 1), 
                                         figure_ct_random.repeat(1, 3, 1, 1).clamp(-1, 1)) 
                self.log(f'validation_lpip_loss', lpips_loss, on_step=True, prog_bar=True, logger=True, sync_dist=True, batch_size=self.batch_size)
                im2d_loss_inv += lpips_loss
            ...
            # Log the final losses
            self.log(f'validation_im2d_loss', im2d_loss, on_step=True, prog_bar=True, logger=True, sync_dist=True, batch_size=self.batch_size)
            self.log(f'validation_im3d_loss', im3d_loss, on_step=True, prog_bar=True, logger=True, sync_dist=True, batch_size=self.batch_size)
    ...
    def configure_optimizers(self):
        ...
        g_opt = torch.optim.AdamW(self.inv_renderer.parameters(), lr=self.lr, betas=(0.9, 0.999))
        d_opt = torch.optim.AdamW(self.unet2d_model.parameters(), lr=self.lr, betas=(0.9, 0.999))
        g_sch = torch.optim.lr_scheduler.MultiStepLR(g_opt, milestones=[100, 200], gamma=0.1)
        d_sch = torch.optim.lr_scheduler.MultiStepLR(d_opt, milestones=[100, 200], gamma=0.1)
