...
class DXRLightningModule(LightningModule):
    def __init__(self, hparams, **kwargs):
        ...
        self.unet3d_model = DiffusionModelUNet(spatial_dims=3, in_channels=1, out_channels=1, num_channels=[24, 48, 96, 192, 384], attention_levels=[False, False, False, True, True], norm_num_groups=24, num_res_blocks=2,)

        self.ddpmsch = DDPMScheduler(num_train_timesteps=self.timesteps, schedule="scaled_linear_beta", prediction_type="v_prediction", beta_start=0.0005, beta_end=0.0195,)
        self.ddimsch = DDIMScheduler(num_train_timesteps=self.timesteps, schedule="scaled_linear_beta", prediction_type="v_prediction", beta_start=0.0005, beta_end=0.0195, clip_sample=False,)
    ...
    def _common_step(self, batch, batch_idx, optimizer_idx, stage: Optional[str] = "evaluation"):
        ...
        # Create timesteps
        timesteps = torch.randint(0, self.inferer.scheduler.num_train_timesteps, (batchsz,), device=_device,).long()  # We are training CT and XR together
        ...
        figure_xr_interp = self.ddpmsch.add_noise(original_samples=image2d, noise=figure_xr_latent, timesteps=timesteps)
        ...
        if self.ddpmsch.prediction_type == "v_prediction":
            ...
            figure_xr_target = self.ddpmsch.get_velocity(image2d, figure_xr_latent, timesteps)
            ...
            figure_xr_target = image2d
        figure_ct_target = self.forward_screen(image3d=volume_ct_target, cameras=view_random)

        # Run the forward
        volume_ct_output = self.unet3d_model(x=volume_ct_interp, timesteps=timesteps)
        # volume_ct_output = self.ddpm_inf(inputs=image3d, diffusion_model=self.unet3d_model, noise=volume_ct_latent, timesteps=timesteps)
        ...
        if batch_idx == 0:
            with torch.no_grad():
                volume_ct_latent = torch.randn_like(image3d)
                volume_ct_output = self.inferer.sample(input_noise=volume_ct_latent, diffusion_model=self.unet3d_model, scheduler=self.ddimsch,)
                figure_ct_output = self.forward_screen(image3d=volume_ct_output, cameras=view_random)
                viz2d = torch.cat([torch.cat([image3d[..., self.vol_shape // 2, :], figure_ct_random, volume_ct_output[..., self.vol_shape // 2, :], figure_ct_output,], dim=-2,).transpose(2, 3),], dim=-2,)
