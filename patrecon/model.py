...
def _make_layers_krlee(in_channels, output_channels, type, batch_norm=False, activation=None):
	layers = []
	...
	if type == 'conv1_s1':
		...
		#layers.append(deconv3x3x3(in_channels, output_channels, stride=1))
		layers.append(torch.nn.Conv3d(in_channels, output_channels, kernel_size=3, stride=1, padding=1))
	elif type == 'deconv4x4_s2':
		layers.append(Upsample(in_channels, output_channels, with_conv=True, scale_factor=2))
	elif type == 'upsample_4':
		layers.append(Upsample(in_channels, output_channels, with_conv=True, scale_factor=(4,1,1)))
	else:
		raise NotImplementedError('layer type [{}] is not implemented'.format(type))
...
class Upsample(nn.Module):
    def __init__(self, in_channels, out_channels, with_conv, scale_factor):
        super().__init__()
        self.scale_factor = scale_factor
        self.with_conv = with_conv
        if self.with_conv:
            self.conv = torch.nn.Conv3d(in_channels, out_channels, kernel_size=3, stride=1, padding=1)

    def forward(self, x):
        x = torch.nn.functional.interpolate(x, scale_factor=self.scale_factor, mode="trilinear")
        if self.with_conv:
            x = self.conv(x)
        return x
...
class ReconNet_modified(nn.Module):

	def __init__(self, in_channels, out_channels, gain=0.02, init_type='standard'):
		super(ReconNet_modified, self).__init__()
		...
		self.trans_layer1 = _make_layers(4096, 		4096, 	'conv1_s1', False, 'relu')
		#self.trans_layer2 = _make_layers(2048, 		2048, 	'deconv1x1_s1', False, 'relu')
		self.trans_layer2 = torch.nn.Conv3d(2048, 2048, kernel_size=1, stride=1, padding=0)

		######### generation network - deconvolution layers
		self.deconv_layer10 = _make_layers_krlee(2048, 	1024, 	'deconv4x4_s2', '3d', 'relu')
		self.deconv_layer8 = _make_layers_krlee(1024,		512, 	'deconv4x4_s2', '3d', 'relu')
		self.deconv_layer7 = _make_layers_krlee(512,		512, 	'deconv3x3_s1', '3d', 'relu')
		self.deconv_layer6 = _make_layers_krlee(512,		256, 	'deconv4x4_s2', '3d', 'relu')
		self.deconv_layer5 = _make_layers_krlee(256,		256, 	'deconv3x3_s1', '3d', 'relu')
		self.deconv_layer4 = _make_layers_krlee(256,		128, 	'deconv4x4_s2', '3d', 'relu')
		self.deconv_layer3 = _make_layers_krlee(128,		128, 	'deconv3x3_s1', '3d', 'relu')
		self.deconv_layer2 = _make_layers_krlee(128,		64, 	'deconv4x4_s2', '3d', 'relu')
		self.deconv_layer1 = _make_layers_krlee(64,		1, 	'deconv3x3_s1', '3d', 'relu')
		#self.deconv_layer0 = _make_layers_krlee(64,		1, 		'upsample_4', '3d', 'relu')
		
		
		#self.output_layer = _make_layers(64,	out_channels, 'conv1_s1', False)
		self.output_layer = nn.Conv3d(1, 1, kernel_size=1, stride=1, padding=0)
...
class ReconNet_modified2(nn.Module):

	def __init__(self, in_channels, out_channels, gain=0.02, init_type='standard'):
		super(ReconNet_modified2, self).__init__()
		...
		self.deconv_layer3 = _make_layers_krlee(128,		128, 	'deconv3x3_s1', '3d', 'relu')
		self.deconv_layer2 = _make_layers_krlee(128,		64, 	'deconv4x4_s2', '3d', 'relu')
		self.deconv_layer1 = _make_layers_krlee(64,		64, 	'deconv3x3_s1', '3d', 'relu')
		self.deconv_layer0 = _make_layers_krlee(64,		1, 		'conv1x1_s1', False, 'relu')
		self.output_layer = _make_layers(64,	out_channels, 'conv1_s1', False)
